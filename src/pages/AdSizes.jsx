const DIGITAL_NOTE =
  'Dimensions are width × height. Supply digital ads at the exact sizes listed, RGB, flat JPG or PNG. Do not supply oversized or double-size files — they are not scaled down.'
...
const AUTO_CHANNEL = [
  {
    ...
    note: DIGITAL_NOTE,
    items: [
      'In-grid banner — 538px × 275px',
      'Rail medium rectangle — 300px × 250px',
      'Rail tall banner — 300px × 600px',
...
function SubsectionGroup({ label, note, items }) {
  return (
    <div className="mb-10">
      ...
      {note && (
        <p className="text-sm text-dim leading-[1.85] mb-4">{note}</p>
      )}
      ...
      <section id="ad-sizes-page" className="bg-off py-16 md:py-[110px] px-5 md:px-[52px] border-t-4 border-t-red">
        <div className="max-w-[1200px] mx-auto">
          ...
          <p className="max-w-[800px] text-[15px] text-dim leading-[1.85] mb-14">
            Dimensions are width × height + bleed as specified. Print files: PDF, CMYK, 300dpi, fonts embedded. Digital files: JPG or PNG, RGB.
          </p>
          ...
          <div className="max-w-[800px]">
            ...
            {AUTO_CHANNEL.map((group) => (
              <SubsectionGroup key={group.label} label={group.label} note={group.note} items={group.items} />
            ))}
